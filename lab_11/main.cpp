...
int main() {
	...
	int length;
	std::cout << "Input len of array (len is nechet): ";
	std::cin >> length;
	while (length % 2 == 0) {
	...
	std::cout << std::endl <<  "Negative num:\n";
	for (int i = 0; i < length; i++) {
		if (*(array + i) < 0) {
			std::cout << *(array + i) 
				<< " addres " << (array + i) << std::endl;
		}
	}
	std::cout << "Num in midle: " << *(array + length / 2) 
		<< " addres " << (array + length / 2) << std::endl;
	double sum = 0;
	int counter = 0;
	for (int i = 0; i < length; i++) {
		if (*(array + i) % 5 == 0) {
